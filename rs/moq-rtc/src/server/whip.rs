...
pub async fn accept(
	...
	// The announcement lives as long as `producer` (held by the IngestSink below);
	// it is unannounced when the producer closes. A false return means the path is
	// outside `publisher`'s scope.
	if !publisher.publish_broadcast(broadcast, consumer) {
		return Err(Error::Other(anyhow::anyhow!(
			"failed to publish broadcast (outside publisher scope)"
		)));
	}
