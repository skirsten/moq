...
That runs **the whole matrix**. There are no CLI flags. The entire invocation lives in `config.yml`. To run a different configuration, copy the file and pass its path:

```bash
bun run --cwd tests/browser test ./config.local.yml
```
...
`.env.local` is `.gitignore`d. Bun loads `.env` / `.env.local` automatically before `run.ts` starts. Credentials are env-only, never in `config.yml`, never on the command line:
...
The whole run is described by `config.yml`, validated with zod on load:
...
`https://moq.dev/watch/` runs the *published* `@moq/watch`. Set `page: local` to test this branch's code instead. The runner is fully self-contained. It:
...
`osVersion` / `device` in `config.yml` match Sauce's trial catalog at the time of writing (iPhone / iOS 26, Samsung S23 FE / Android 16). When a provider rotates its fleet a run fails with the available candidates listed. Update `config.yml`.

To see what each cloud offers *before* a run, print the live catalogs:

```bash
bun run --cwd tests/browser catalog                 # both providers
bun run --cwd tests/browser catalog sauce            # just Sauce Labs
bun run --cwd tests/browser catalog browserstack     # just BrowserStack
bun run --cwd tests/browser catalog -- --region us-west-1
```

It pulls each provider's REST catalog and prints `browser` / `os` / `osVersion` for desktop and the real-device list for mobile, grouped the way `config.yml` targets are written. Browse only: it never validates a config or runs a test. Sauce desktop browsers are public; Sauce real devices and the BrowserStack catalog use the same credentials as `run.ts` (a section with missing credentials is skipped). Note BrowserStack reports macOS as release names (`Ventura`, `Sonoma`), so a `browserstack` macOS target needs `osVersion: "Ventura"`, not a number.
