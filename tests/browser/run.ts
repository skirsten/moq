...
//   bun run.ts              -> runs ./config.yaml  (one command, whole matrix)
//   bun run.ts other.yaml   -> runs a different config
...
// config.yaml's `targets` is the exact run list. Drop ones their provider can't run
