...
- `Received RESET_STREAM` on the catalog subscribe (means the broadcast isn't actually announced at this relay path; check `relayUrl`/`broadcast` in `config.yaml`).
